...
int calc_crc(const char *s, const int len) {
	int chksum = 0;
	int it;
	...
	{
		chksum ^= (int) s[it];
		printf("[CS] char : %c, int : %d, current checksum : %d\n", s[it], (int) s[it], chksum);
	}
...
int main(int argc __attribute__((unused)), char *argv[] __attribute__((unused))) {
	...
	for (it = 0; it < 10; it++) {
		...
		gen_sz = nmea_generate(&buff[0], 2048, &info, GPRMC);

		buff[gen_sz] = 0;
		printf("%s\n", &buff[0]);
		printf("CHKSUM6 %x\n", calc_crc(&buff[0] + 1, gen_sz - 6));
